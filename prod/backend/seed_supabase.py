...
for tmpl in templates:
    ...
    for _ in range(max(1, count)):
        ...
        insert_row = {
            ...
            "location_bucket": data["location_bucket"],
            "evidence_span": issue_text[:15] + "...",
            "raw_log": {"generated_by": "seed script"}
        }
        
        bulk_insert_data.append(insert_row)

print(f"Prepared {len(bulk_insert_data)} entries. Inserting into Supabase...")

try:
    # We can insert in batche if there are too many, but 200 is fine for one chunk
    res = supabase.table("interviews").insert(bulk_insert_data).execute()
    print("Success! Inserted test data.")
