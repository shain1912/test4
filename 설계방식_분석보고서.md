...
### 7.3 하이브리드 시각화 전략: 거시적 조망과 미시적 해석의 결합

단순한 3D 시각화는 "멋져 보이지만 해석하기 어렵다"는 한계가 있다. 본 시스템은 이를 극복하기 위해 **이중 시각화 전략(Dual-View Strategy)**을 채택했다.

1.  **거시적 조망 (Macro View - 3D Map)**: t-SNE로 투영된 3차원 공간에서 클러스터 간의 거리와 밀도를 통해 전체적인 여론의 지형을 파악한다. (예: "서면 안전 이슈와 해운대 접근성 이슈가 서로 멀리 떨어져 있군")
2.  **미시적 해석 (Micro View - Topic Cards)**: 3D 지도 하단에 **'토픽 상세 카드'**를 배치하여, 각 군집의 **핵심 키워드, 심각도 평균, 실제 시민의 목소리(Verbatim)**를 즉시 확인할 수 있게 한다.

이는 데이터의 **'수학적 구조'**와 **'인문학적 의미'**를 동시에 전달하는 가장 효과적인 UX 설계이다.
